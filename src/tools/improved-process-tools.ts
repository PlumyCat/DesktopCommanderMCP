 ...
 * 
 * NOTE: Sessions persist only while the process is running:
 * - Interactive shells/REPLs: Session stays active for further interaction
 * - Simple commands (echo, ls, etc.): Session closes when command completes
 * - Long-running processes: Session stays active until process ends
 * 
 * Use start_process with interactive shells for persistent sessions.
 */
 ...
 * Provides better visibility into session lifecycle
 */
export async function listSessions(): Promise<ServerResult> {
  const activeSessions = terminalManager.listActiveSessions();
  ...
  if (activeSessions.length > 0) {
    ...
    response += activeSessions.map(s =>
      `  PID: ${s.pid} | Status: ${s.isBlocked ? 'Waiting for input' : 'Running'} | Runtime: ${Math.round(s.runtime / 1000)}s`
    ).join('\n');
  } else {
    response += '🟢 ACTIVE SESSIONS: None\n';
  }

  // Recently completed sessions (last 10)
  response += '\n\n🔴 RECENTLY COMPLETED SESSIONS:\n';
  if (completedSessions.length > 0) {
    const recentSessions = completedSessions.slice(-10).reverse(); // Show last 10, most recent first
    response += recentSessions.map(s => {
      const runtime = ((s.endTime.getTime() - s.startTime.getTime()) / 1000).toFixed(1);
      const exitStatus = s.exitCode === 0 ? '✅' : s.exitCode === null ? '⚠️' : '❌';
      return `  PID: ${s.pid} | Exit: ${s.exitCode ?? 'N/A'} ${exitStatus} | Runtime: ${runtime}s | Ended: ${s.endTime.toLocaleTimeString()}`;
    }).join('\n');
  } else {
    response += '  None';
  }

  response += '\n\n💡 TIP: Sessions automatically close when commands complete. Use interactive shells (bash, python, etc.) for persistent sessions.';
